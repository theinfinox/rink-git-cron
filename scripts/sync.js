        ...
        try {
            ...
            for (let tIndex = 0; tIndex < tabsToFetch.length; tIndex++) {
                ...
                for (let i = 1; i < rows.length; i++) {
                    ...
                    if (tab.autoGenerateId && tab.autoGenerateId.enabled) {
                        const idField = tab.autoGenerateId.field || 'id';
                        if (!rowObj[idField] || rowObj[idField] === '') {
                            const startNum = Number(tab.autoGenerateId.start) || 100001;
                            rowObj[idField] = `${tab.autoGenerateId.prefix || 'id'}_${startNum + autoGeneratedCount}`;
                            autoGeneratedCount++;
                        }
                    }
